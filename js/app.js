...
window.onload = async () => {
    console.log("ready...")
    // init chart on page loaded first time
    initTheme()
...
loadAllTimeChart = async (country) => {
    let labels = []
    let confrim_data, recovered_data, deaths_data
    ...
    if (isGlobal(country)) {
        ...
        confrim_data = renderWorldData(world_data, CASE_STATUS.confirmed)
        recovered_data = renderWorldData(world_data, CASE_STATUS.recovered)
        deaths_data = renderWorldData(world_data, CASE_STATUS.deaths)
    } else {
        let confrimed = await covidApi.getCountryAllTimeCases(country, CASE_STATUS.confirmed)
        ...
        confrim_data = renderData(confrimed)
...
renderCountrySelectList = (list) => {
    let countr_select_list = document.querySelector('#country-select-list')
    countr_select_list.querySelectorAll('div').forEach(e => e.remove())
    list.forEach(e => {
        ...
        item.onclick = async () => {
            ...
            countr_select_list.classList.toggle('active')
            console.log(e.Slug)
            await loadData(e.Slug)
        }
        countr_select_list.appendChild(item)
    })
}
loadCountrySelectList = async () => {
    ...
    item.onclick = async () => {
        document.querySelector('#countrys-select-span').textContent = 'Global'
        countr_select_list.classList.toggle('active')
        await loadData('Global')
    }
